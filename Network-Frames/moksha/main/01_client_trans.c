...
static void sub_bytes(uint8_t *s) {
    for (int i = 0; i < 16; i++) s[i] = s_box[s[i]];
}
...
static void mix_columns(uint8_t *s) {
    ...
    for (int c = 0; c < 4; c++) {
        int i = c * 4;
        t[i]   = gmult(2,s[i]) ^ gmult(3,s[i+1]) ^ s[i+2] ^ s[i+3];
        t[i+1] = s[i] ^ gmult(2,s[i+1]) ^ gmult(3,s[i+2]) ^ s[i+3];
        t[i+2] = s[i] ^ s[i+1] ^ gmult(2,s[i+2]) ^ gmult(3,s[i+3]);
        t[i+3] = gmult(3,s[i]) ^ s[i+1] ^ s[i+2] ^ gmult(2,s[i+3]);
    }
    for (int i = 0; i < 16; i++) s[i] = t[i];
}

static void add_round_key(uint8_t *s, uint8_t *rk) {
    for (int i = 0; i < 16; i++) s[i] ^= rk[i];
}

/* ===================== KEY EXPANSION ===================== */
static void key_expansion(uint8_t *key, uint8_t *rk) {
